...
class DualRTE:
    ...
    def __init__(
        ...
        self.state_gradient = copy.deepcopy(expectation_gradient)
    ...
    def get_loss_and_gradient(self, dt: float, theta: np.ndarray):
        # partial_energy_gradient = self.expectation_gradient.compute(theta)
        # partial_state_gradient = self.state_gradient.compute(theta)
        rev = ReverseGradient(partial_gradient=True)
        hamiltonian = self.expectation.hamiltonian
        iden = SparsePauliOp(["I" * hamiltonian.num_qubits])
        circuit = self.ansatz
        partial_energy_gradient = rev.compute(hamiltonian, circuit, theta)
        partial_state_gradient = rev.compute(iden, circuit, theta)
        energy = self.expectation.evaluate(theta)

        # sign is swapped here due to the minus sign in the QNG update step
        energy_gradient = -np.imag(partial_energy_gradient - energy * partial_state_gradient)
    ...
    def evolve(
        self, hamiltonian: PauliSumOp, final_time: float, timestep: float, observable: PauliSumOp
    ) -> DualRTEResult:
        ...
        observable_evaluator = copy.deepcopy(self.expectation)
        observable_evaluator.hamiltonian = observable
        ...
        losses = []
        gradients = []
        observable_values = [observable_evaluator.evaluate(self.initial_parameters)]
        ...
        for i in range(num_timesteps):
            ...
            observable_values.append(observable_evaluator.evaluate(next_theta))
        ...
        return DualRTEResult(
            times, parameters, energies, observable_values, thetas, losses, gradients
        )
    ...
    def _sync_primitives(self, hamiltonian):
        ...
        identity = PauliSumOp(SparsePauliOp(["I" * hamiltonian.num_qubits]))
        self.state_gradient.expectation = copy.deepcopy(self.expectation)
        self.state_gradient.expectation.hamiltonian = identity

        # need the partial gradient for real time evolution
        self.expectation_gradient.partial = True
