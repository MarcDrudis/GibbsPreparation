...
class HamiltonianLearning:
    ...
    def __init__(self,state: Statevector|QuantumCircuit|DensityMatrix , k_learning:int, k_constraints:int, parameters:None|list[np.ndarray] = None, periodic:bool = False) -> None:
        ...
        self.learning_basis = KLocalPauliBasis(k=k_learning, num_qubits = self.num_qubits, periodic = periodic)
        self.constraint_basis = KLocalPauliBasis(k=k_constraints, num_qubits = self.num_qubits, periodic = periodic)
        self.sampling_basis = KLocalPauliBasis(k=k_learning+k_constraints-1, num_qubits = self.num_qubits, periodic = periodic)
    ...
    def classical_learn_hamiltonian(self,i:int=-1):
        ...
        recov_vec = [np.trace(hamiltonian_cl_rec@Pauli(p).to_matrix()) for p in self.learning_basis._paulis_list]
    ...
    def project_hamiltonian(self,l:int, hamiltonian:SparsePauliOp):
        ...
        for i in range(l):
            projection_coeffs[i] = np.real(np.dot(self.singular_decomposition[i][1],original_vector))
