...
def dataframe(gibbs_result:GibbsResult):
    local_sizes = np.array([0]+[KLocalPauliBasis(i+1,gibbs_result.num_qubits).size for i in range(gibbs_result.klocality)])
    ...
    data = {
        "Pauli Basis Label":KLocalPauliBasis(gibbs_result.klocality,gibbs_result.num_qubits)._paulis_list,
        "Pauli Basis":np.arange(KLocalPauliBasis(gibbs_result.klocality,gibbs_result.num_qubits).size),
        "locality":local_labels,
        "prepH":np.real(gibbs_result.cfaulties[-1]),
        "priorH":np.real(gibbs_result.coriginal)*gibbs_result.betas[-1],
    }
...
def posteriordist(gibbs_result:GibbsResult,posterior_mean,posterior_cov):
    ...
    k_locality = gibbs_result.klocality
    c_original_prior = gibbs_result.coriginal
    data,local_sizes = dataframe(gibbs_result)
    ...
    fig = px.bar(data, x='Pauli Basis', y=["Prior Error","Posterior Error"],color_discrete_sequence=["blue", "red"],
                barmode="group", hover_name="Pauli Basis Label"
                )
    fig.add_trace(
        go.Scatter(
...
def preparation(result:GibbsResult)->None:
    import plotly.express as px
    df,local_sizes = dataframe(result)
    fig = px.bar(df, x='Pauli Basis', y=["prepH","priorH"],
                barmode="group", hover_name="Pauli Basis Label"
                )
...
def preparation_vsclassic(result:GibbsResult,result_classic:GibbsResult)->None:
    ...
    fig = px.bar(df, x='Pauli Basis', y=["prepH","priorH","classicprepH"],
                barmode="group", hover_name="Pauli Basis Label"
                )
    axis_locality(fig,result.klocality,local_sizes)
    fig.update_layout(
...
def compare_preparations(results = list[GibbsResult],titles = list[str]):
    ...
    fig = px.bar(df, x='Pauli Basis', y=["priorH"]+titles,
                barmode="group", hover_name="Pauli Basis Label"
                )
...
def preparation_error(result:GibbsResult)->None:
    ...
    fig = px.bar(df, x='Pauli Basis', y="Preparation Error",
                barmode="group", hover_name="Pauli Basis Label"
                )
    axis_locality(fig,result.klocality,local_sizes)
    fig.update_layout(
