...
onelocalsize = KLocalPauliBasis(1, num_qubits).size
horiginal_base = lattice_hamiltonian(**lattice_hamiltonian_arguments)
coriginal_base = kbasis.pauli_to_vector(horiginal_base)
control_field = np.zeros_like(coriginal_base)
control_field[:onelocalsize] = np.random.uniform(-0.1, 0.1, onelocalsize)
coriginal = coriginal_base + control_field
