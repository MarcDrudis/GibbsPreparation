...
class RPGate(Gate):
    ...
    def __init__(self,pauli:str, theta: ParameterValueType, label: Optional[str] = None):
        """Create new RPauli gate."""
        super().__init__("RP", len(pauli), [theta], label=label)
        self.pauli = pauli
    ...
    def _define(self):
        ...
        basis_rules = [(basis_rotation[p],[q[i]],[]) for i,p in enumerate(self.pauli) if basis_rotation[p] is not None]
        
        #The entanglement will be performed only on non-trivial qubits.
        ent_rules = [(CXGate(), [q[i], q[rotation_qubit]], []) for i in range(rotation_qubit) if self.pauli[i] != "I" ]
